...
def fetch_system_info(addr):
    ...
    return int(system_info[0,1]), channels, int(system_info[2,1])
...
def extract_seizure_data(src, des, preictal_info, interictal_info, sampling_freq = 256, time_window = 10, overlap = 0.75):
    ...
    for file_sno in range(len(seizure_filename)):
        ...
        if  (start_time[file_sno]-(prediction_start_time[file_sno]*60) ) >= 0:
            ...
            print ('Reading from ',(src +'\\' + seizure_filename[file_sno]))
        ...
        if type(seizure_prev_file) != str:
            ...
            if ((start_time[file_sno] - prediction_start_time[file_sno]*60) > 0):
                ...
                seizure = seizure_file[:,int(start_index): int((start_time[file_sno]-300)*sampling_freq)]
        ...
        while(prediction_start_time[file_sno]>5):
            ...
            seizure_block = seizure[:,(count*300*sampling_freq):int((count+1)*300*sampling_freq)]
    ...
    for file_sno in range(len(interictal_info)):
        ...
        if round(num_of_splits) != num_of_splits:
            ...
            seizure_file = seizure_file[:,0:int(seizure_file.shape[1] - trim_value)]
...
def createDataset(addr, start_time, end_time, num_of_channels = 23, sampling_freq = 256, channels = None, time_window = 10):
    '''
