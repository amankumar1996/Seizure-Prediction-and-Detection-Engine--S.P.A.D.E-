...
class Epoc(object):
    ...
    def connect(self, timeout = 10):   
        ...
        self.edk.IEE_EngineConnect(connect_param)
        self.data_handler = self.edk.IEE_DataCreate()
        self.edk.IEE_DataSetBufferSizeInSec(5)
        
        eEvent = self.edk.IEE_EmoEngineEventCreate()
        state = self.edk.IEE_EngineGetNextEvent(eEvent)
        t0 = time.time()
        while not self.connected:
            state = self.edk.IEE_EngineGetNextEvent(eEvent)
            if not state:
                self.connected = True
                self.edk.IEE_DataAcquisitionEnable(c_uint(0),c_bool(1))
                break
    ...
    def aquire(self,idx):
        nSamples = c_int()
        while True:
            self.edk.IEE_DataUpdateHandle(c_uint(0), self.data_handler)
            self.edk.IEE_DataGetNumberOfSample(self.data_handler, 
                                              byref(nSamples))
            n = nSamples.value
            if not n:
                continue
            ...
            for i in idx:
                ...
                self.edk.IEE_DataGet(self.data_handler, i, byref(data_ctype),
                                    c_uint(n))
