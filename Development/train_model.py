...
preictal_addr = ''
interictal_addr = ''
preictal_feature_data = (pd.read_csv(,
                                    header = None)).as_matrix()

interictal_feature_data = (pd.read_csv(,
                                    header = None)).as_matrix()
...
for i in range(corres_num_of_epochs.shape[0]-1):
    ...
    for channel in channels:
        channel_features.extend(range((channel-1)*num_of_features_per_channel,channel * num_of_features_per_channel))
    ...
    end_time = time()
    print 'Time taken to train : ',(end_time-start_time)/60
    ...
    print '----------------  Single channel = 8, ', folder_names[i], '\n'
